...
DASHBOARD_PASS = os.getenv("DASH_PASS", "123456789")   # ← cambiar antes de desplegar
FILAS_POR_PAG  = 20
...
def dashboard():
    ...
    where, params = [], []
    if filtro_desde:
        where.append("fecha >= ?"); params.append(filtro_desde)
    if filtro_hasta:
        where.append("fecha <= ?"); params.append(filtro_hasta)
    ...
    # Filas paginadas
    filas = conn.execute(
        f"""SELECT * FROM lecturas {where_sql}

...
def db_init(path: str) -> sqlite3.Connection:
    ...
    # ═══ NUEVO: TABLA PARA PREDICCIONES IA ═══
    conn.execute("""
...
def hilo_ubidots():
    """Hilo separado: cada UBIDOTS_INTERVALO_S envía a Ubidots."""
    while True:
...
# ═══ NUEVO: MÓDULO DE IA INTEGRADO ═══════════════════════════
def ejecutar_prediccion_ia():
    """Ejecuta el módulo de IA y guarda resultados en SQLite."""
    try:
        # Importar ia_module dinámicamente
        spec = importlib.util.spec_from_file_location("ia_module", "ia_module.py")
        ia = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(ia)
        
        # Obtener lecturas
        lecturas = ia.obtener_ultimas_lecturas(horas=24, limite=100)
        if len(lecturas) < 10:
            log.warning("Insuficientes datos para predicción IA")
            return
        
        # Generar predicción
        prediccion = ia.generar_prediccion(lecturas)
        
        # Generar recomendaciones
        recomendaciones = ia.generar_recomendaciones(lecturas, prediccion)
        
        # Guardar en tabla predictions
        conn = sqlite3.connect(CONF["db_path"])
        conn.execute("""
            INSERT INTO predictions 
            (timestamp, prediccion_6h, pm25_estimado, confianza, razonamiento, 
             prioridad, acciones, comunicacion)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            int(time.time()),
            prediccion.get('prediccion_6h', 'DESCONOCIDA'),
            prediccion.get('pm25_estimado', 0),
            prediccion.get('confianza', 'BAJA'),
            prediccion.get('razonamiento', ''),
            recomendaciones.get('prioridad', 'MEDIA'),
            json.dumps(recomendaciones.get('acciones_inmediatas', [])),
            recomendaciones.get('comunicacion_publica', '')
        ))
        conn.commit()
        conn.close()
        
        log.info("Predicción IA guardada ✓")
        
    except Exception as e:
        log.error(f"Error en predicción IA: {e}")


def hilo_ia():
    """Ejecuta predicciones cada 30 minutos."""
    while True:
        time.sleep(1800)  # 30 minutos
        ejecutar_prediccion_ia()
# ═══════════════════════════════════════════════════════════
...
def main():
    ...
    # ═══ NUEVO: HILO DE IA ═══════════════════════════════
    t_ia = threading.Thread(target=hilo_ia, daemon=True, name="ia-predictor")
    t_ia.start()
    log.info("Hilo IA iniciado (predicción cada 30 min)")

    # Ejecutar primera predicción inmediatamente
    threading.Thread(target=ejecutar_prediccion_ia, daemon=True).start()
    # ═════════════════════════════════════════════════════
    ...
    try:
        try:
            ...
            log.error("Error al conectar al broker MQTT en %s:%d - %s", 
                     CONF["mqtt_host"], CONF["mqtt_port"], e)
            return
